  ...
  const validateExpenses = (): boolean => {
    for (const exp of editedExpenses) {
      ...
      if (exp.sasia === null || exp.sasia <= 0) {
        showError(`Quantity (Sasia) must be greater than 0.`);
        return false;
      }
      if (!exp.njesia || !NJESIA_OPTIONS.includes(exp.njesia)) {
        showError(`Unit (Njesia) is required.`);
        return false;
      }
    }
    return true;
          ...
          <div className="lg:col-span-3 min-w-0">
            <div className="bg-card border rounded-lg shadow-sm p-6">
              ...
              >
                ...
                <Button onClick={addNewExpense} variant="secondary" className="w-full mt-4" disabled={isLoading}>
