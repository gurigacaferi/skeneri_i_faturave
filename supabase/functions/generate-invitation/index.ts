...
serve(async (req) => {
  ...
  try {
    ...
    if (!resendApiKey) {
      console.error('RESEND_API_KEY is missing from environment variables. Invitation created, but email not sent.');
      // Continue execution but log the error, as the invitation is already created
    } else {
      const resend = new Resend(resendApiKey);
      
      // Construct the sign-up link to pre-fill the form
      const origin = req.headers.get('origin') || 'http://localhost:8080';
      const signupLink = `${origin}/login?tab=signup&code=${code}&email=${email}`;
      ...
      try {
        await resend.emails.send({
          ...
          html: `
            ...
            <p>If the link doesn't work, use the code <strong>${code}</strong> at the sign-up page.</p>
          `,
        });
