...
async function fileToBase64Images(file: Blob): Promise<string[]> {
  ...
  if (isPdf) {
    ...
    // Since we can't use canvas in Deno, we'll send the PDF as is with proper mime type
    const base64 = btoa(String.fromCharCode(...bytes));
    return [`data:application/pdf;base64,${base64}`];
  } else {
    // For images, detect mime type
...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`Processing receipt ${receiptId} from ${storagePath}`);
    ...
    const base64Images = await fileToBase64Images(fileData);
    ...
    base64Images.forEach((base64Image: string) => {
    ...
    // Validate and ensure categories
    ...
    if (content.expenses && Array.isArray(content.expenses)) {
      content.expenses = content.expenses.map((expense: any) => {
        ...
        if (!category.trim() || !VALID_CATEGORIES.includes(category.trim())) {
          console.warn(`Invalid category for ${expense.name}, using fallback`);
    ...
    if (content.expenses && content.expenses.length > 0) {
      ...
      console.log(`Successfully processed receipt ${receiptId} with ${content.expenses.length} expenses`);
    } else {
      await supabase
        .from('receipts')
        .update({ 
      ...
      console.log(`No expenses found for receipt ${receiptId}`);
    }
