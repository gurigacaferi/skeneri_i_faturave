...
serve(async (req) => {
  ...
  try {
    ...
    // Trigger the processing function asynchronously (fire and forget)
    // This will run independently of the client connection
    fetch(`${Deno.env.get('SUPABASE_URL')}/functions/v1/process-receipt-async`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${Deno.env.get('SUPABASE_SERVICE_ROLE_KEY')}`,
      },
      body: JSON.stringify({ receiptId, storagePath: receipt.storage_path }),
    }).catch(error => {
